...
class BluetoothViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // --- NUEVO: flujo para la bola entrante ---
    ...
    private fun onConnected() {
        ...
        myNickname?.let { sendData("$NICK_HEADER$APP_HEADER$it".toByteArray(Charsets.UTF_8)) }
    }
    ...
    fun startServer(nickname: String) { myNickname = nickname; isHost = true; connection?.startServer() }
    fun connect(device: BluetoothDevice, nickname: String) { myNickname = nickname; isHost = false; stopDiscovery(); connection?.connectToDevice(device) }
    fun sendData(data: ByteArray) { connection?.sendBytes(data) }
    fun sendStartSignal() { sendData(START_HEADER.toByteArray(Charsets.UTF_8)); _startSignalReceived.value = true }
